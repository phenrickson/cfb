...
The visualization below shows the simulated paths for **`r params$team`** as of Week `r params$sim_week` in the `r params$input_season` season. This illustrates the most common paths left in a team's season by showing how the result of one game tends to affect its future trajectory.
...
p+
          ggtitle(paste("Simulated Elo Ratings for" , params$team, params$input_season, "after Week", params$sim_week),
...
week_games = c(games %>% filter(HOME_TEAM == params$team | AWAY_TEAM == params$team) %>% 
        filter(!is.na(HOME_POINTS)) %>% pull(WEEK),0)
...
p = plot_elo_team_season(sim_from_start%>%
                     ...
                     params$input_season,
                     alpha = 0.05)+
        geom_line(data = team_predictions %>%
                          filter(TEAM == params$team) %>%
                          filter(SIM_FROM_WEEK %in% week_games) %>%
                          arrange(SIM_FROM_WEEK) %>%
                          group_by(SEASON, TEAM) %>%
                          mutate(GAME = row_number()) %>%
                          ungroup() %>%
                          mutate(.id = params$nsims+1) %>%
                          select(.id, GAME, SEASON, TEAM, RATING) %>%
