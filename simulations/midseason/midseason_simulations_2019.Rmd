---
title: "Midseason Simulations for the CFB Season"
...
conference_divisions = 
        tibble(SEASON = params$input_season,
               CONFERENCE = "SEC",
...
This notebook details results from simulating the remaining games in the `r params$input_season` college football season. We use historical college football play by play and game data in order to develop a model for simulating upcoming games. All data is from collegefootballdata.com.
...
# simulate season after each week
set.seed(1999)
midseason_simulations = sim_rest_of_season(games,
                                           season = params$input_season,
...
games_so_far = 
        ...
        mutate(HOME_OUTCOME = case_when(HOME_POINTS > AWAY_POINTS ~ 'win',
                                        TRUE ~ 'loss'),
               AWAY_OUTCOME = case_when(HOME_POINTS < AWAY_POINTS ~ 'win',
                                        TRUE ~ 'loss')) %>%
        select(GAME_ID,
               SEASON,
               SEASON_TYPE,
        ...
        left_join(.,
                  conference_divisions %>%
                          rename(OPPONENT = TEAM,
                                 OPPONENT_CONFERENCE = CONFERENCE,
                                 OPPONENT_CONFERENCE_DIVISION = CONFERENCE_DIVISION),
                  by = c("SEASON", "OPPONENT", "OPPONENT_CONFERENCE")) %>%
        mutate(CONFERENCE_GAME = case_when(CONFERENCE == OPPONENT_CONFERENCE ~ T,
                                           TRUE ~ F),
               CONFERENCE_DIVISION_GAME = case_when(CONFERENCE == OPPONENT_CONFERENCE & CONFERENCE_DIVISION == OPPONENT_CONFERENCE_DIVISION ~ T,
                                                    CONFERENCE == OPPONENT_CONFERENCE & ((is.na(CONFERENCE_DIVISION) & is.na(OPPONENT_CONFERENCE_DIVISION))) ~ T,
...
# # get simulated wins left
# sim_remaining_wins = sim_remaining_games %>%
#         # filter(CONFERENCE_DIVISION_GAME == T) %>%
#         group_by(.id, SIM_FROM_WEEK, SIM_FROM_DATE, SEASON, TEAM, CONFERENCE, CONFERENCE_DIVISION, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, SIM_OUTCOME) %>%
#         count() %>%
#         ungroup() %>%
#         spread(SIM_OUTCOME, n) %>%
#         mutate_at(c("win", "loss"),
#                   ~ replace_na(., 0)) %>%
#         select(.id, SIM_FROM_WEEK, SIM_FROM_DATE, SEASON, TEAM, CONFERENCE, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, win, loss) %>%
#         mutate(WEEK = SIM_FROM_WEEK) %>%
#         mutate_at(c("win", "loss"),
#                   ~ replace_na(., 0)) %>%
#         rename(sim_remaining_wins = win,
#                sim_remaining_loss = loss) %>%
#         as_tibble()
# 
# # get wins so far
# wins_so_far = season_weeks_teams %>%
#         left_join(.,
#                   games_so_far  %>%
#                           mutate(CONFERENCE_GAME = CONFERENCE == OPPONENT_CONFERENCE) %>%
#                           group_by(SEASON, TEAM, WEEK, GAME_DATE, CONFERENCE, CONFERENCE_DIVISION, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, OUTCOME) %>%
#                           count() %>%
#                           spread(OUTCOME, n) %>%
#                           mutate_at(c("win", "loss"),
#                                     ~ replace_na(., 0)) %>%
#                           # group_by(SEASON, TEAM) %>%
#                           # mutate(win = cumsum(win),
#                           #        loss = cumsum(loss)) %>%
#                           select(SEASON, TEAM, WEEK, GAME_DATE, CONFERENCE, CONFERENCE_DIVISION, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, win, loss) %>%
#                           ungroup(),
...
sim_remaining_games %>%
        group_by(.id, SIM_FROM_WEEK, TEAM, CONFERENCE, CONFERENCE_DIVISION) %>% 
        mutate(win = case_when(SIM_OUTCOME == 'win' ~ 1, TRUE ~ 0),
               division_win = case_when(SIM_OUTCOME == 'win' & CONFERENCE_DIVISION_GAME == T ~ 1,
                                        TRUE ~ 0),
               conference_win = case_when(SIM_OUTCOME == 'win' & CONFERENCE_GAME == T ~ 1,
                                          TRUE ~ 0)) %>%
        ...
        mutate(WEEK = SIM_FROM_WEEK,
               type = 'sim') %>%
        select(type, .id, WEEK, everything()) %>%
        pivot_wider(., id_cols = c("WEEK","SIM_FROM_WEEK","TEAM", "CONFERENCE", "CONFERENCE_DIVISION"),
                    names_from = c("type"),
                    values_from = c("wins", "conference_wins", "division_wins"))

games_so_far %>%
        group_by(WEEK, TEAM, CONFERENCE, CONFERENCE_DIVISION) %>% 
        mutate(win = case_when(OUTCOME == 'win' ~ 1, TRUE ~ 0),
               division_win = case_when(OUTCOME == 'win' & CONFERENCE_DIVISION_GAME == T ~ 1,
                                        TRUE ~ 0),
               conference_win = case_when(OUTCOME == 'win' & CONFERENCE_GAME == T ~ 1,
                                          TRUE ~ 0)) %>%
        summarize(wins = sum(win),
                  conference_wins = sum(conference_win),
                  division_wins = sum(division_win),
        ...
        group_by(type, TEAM) %>%
        mutate(wins = cumsum(wins),
               conference_wins = cumsum(conference_wins),
               division_wins = cumsum(division_wins)) %>%
        pivot_wider(., id_cols = c("WEEK", "TEAM", "CONFERENCE", "CONFERENCE_DIVISION"),
                    names_from = c("type"),
                    values_from = c("wins", "conference_wins", "division_wins"))

```
Join this up and then compute winners


```{r check on conference}

actual_conference_wins = 
        season_weeks_teams %>%
        left_join(.,
                  games %>%
                          filter(SEASON == params$input_season) %>%
                          ...
                          bind_rows(.,
                                    games %>%
                                            ...
                                            set_names(., gsub("AWAY_", "", names(.)))) %>%
                          mutate(CONFERENCE_GAME = CONFERENCE == OPPONENT_CONFERENCE) %>%
                          group_by(SEASON, TEAM, WEEK, GAME_DATE, CONFERENCE_GAME, OUTCOME) %>%
                          count() %>%
                          spread(OUTCOME, n) %>%
                          mutate_at(c("win", "loss"),
                                    ~ replace_na(., 0)) %>%
                          ...
                          # mutate(win = cumsum(win),
                          #        loss = cumsum(loss)) %>%
                          select(SEASON, TEAM, WEEK, GAME_DATE, CONFERENCE_GAME, win, loss) %>%
                          ungroup(),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        as_tibble() %>%
        mutate_at(c("win", "loss"),
...
# now aggregate and then combine
season_weeks_teams %>%
        left_join(., 
                  sim_conference_wins %>%
                         # filter(CONFERENCE == 'SEC') %>%
                          arrange(SIM_FROM_WEEK) %>%
                          group_by(.id, SEASON, TEAM, SIM_FROM_WEEK) %>%
                          summarize(sim_remaining_wins = mean(sim_remaining_wins),
                                    sim_remaining_loss = mean(sim_remaining_loss),
                                    .groups = 'drop') %>%
                          group_by(SEASON, TEAM, SIM_FROM_WEEK) %>%
                          summarize(sim_remaining_wins = mean(sim_remaining_wins),
                                    sim_remaining_loss = mean(sim_remaining_loss),
        ...
        left_join(.,
                  actual_conference_wins %>%
                          select(SEASON, WEEK, TEAM, wins_so_far, loss_so_far),
                  by = c("WEEK",
                         "SEASON", 
                         "TEAM")) %>%
        mutate_at(c("sim_remaining_wins",
                    "sim_remaining_loss"),
                  ~ replace_na(., 0)) %>%
        mutate(expected_wins = sim_remaining_wins + wins_so_far) %>%
        ...
        custom_color_teams+
        guides(color = 'none')+
        facet_wrap(CONFERENCE_DIVISION ~.)+
...
```{r get simulated wins and conference winners}
...
wins_so_far = 
        ...
        left_join(.,
                  games_so_far %>%
                          group_by(SEASON, WEEK, TEAM, OUTCOME) %>%
                          count() %>%
                          spread(OUTCOME, n),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        mutate_at(c("win", "loss"),
...
# running division wins
conference_wins_so_far = 
        season_weeks_teams %>%
        left_join(.,
                  games_so_far %>%
                          filter(CONFERENCE_GAME == T) %>%
                          group_by(SEASON, WEEK, TEAM, CONFERENCE_GAME, OUTCOME) %>%
                          count() %>%
                          spread(OUTCOME, n),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        mutate_at(c("win", "loss"),
                  ~ replace_na(., 0)) %>%
        group_by(SEASON, TEAM) %>%
        mutate(conference_wins_so_far = cumsum(win),
               conference_loss_so_far = cumsum(loss)) %>%
        ungroup() %>%
        select(-win, -loss) %>%
        select(SEASON, WEEK, TEAM, conference_wins_so_far, conference_loss_so_far)

division_wins_so_far = 
        season_weeks_teams %>%
        left_join(.,
                  games_so_far %>%
                          filter(CONFERENCE_DIVISION_GAME == T) %>%
                          group_by(SEASON, WEEK, TEAM, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, OUTCOME) %>%
                          count() %>%
                          spread(OUTCOME, n),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        mutate_at(c("win", "loss"),
                  ~ replace_na(., 0)) %>%
        group_by(SEASON, TEAM) %>%
        mutate(division_wins_so_far = cumsum(win),
               division_loss_so_far = cumsum(loss)) %>%
        ungroup() %>%
        select(-win, -loss) %>%
        select(SEASON, WEEK, TEAM, division_wins_so_far, division_loss_so_far)


total_wins_so_far = wins_so_far %>%
        left_join(., division_wins_so_far,
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        left_join(., conference_wins_so_far,
                  by = c("SEASON", "WEEK", "TEAM"))
...
sim_remaining_wins  = 
        ...
        left_join(.,
                sim_remaining_games %>%
                          group_by(.id, SEASON, SIM_FROM_WEEK, TEAM, SIM_OUTCOME) %>%
                          count() %>%
                          spread(SIM_OUTCOME, n) %>%
                          mutate_at(c("win", "loss"),
                                            ~ replace_na(., 0)) %>%
        ...
        left_join(.,
                sim_remaining_games %>%
                          ...
                          mutate_at(c("win", "loss"),
                                            ~ replace_na(., 0)) %>%
                          rename(division_sim_remaining_wins = win,
                                 division_sim_remaining_loss = loss) %>%
                          mutate(WEEK = SIM_FROM_WEEK),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        left_join(.,
                sim_remaining_games %>%
                          ...
                          mutate_at(c("win", "loss"),
                                            ~ replace_na(., 0)) %>%
                          rename(conference_sim_remaining_wins = win,
                                 conference_sim_remaining_loss = loss) %>%
                          mutate(WEEK = SIM_FROM_WEEK),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        mutate_at(c(
                "sim_remaining_wins", "sim_remaining_loss",
                "division_sim_remaining_wins", "conference_sim_remaining_loss",
                "conference_sim_remaining_wins", "conference_sim_remaining_loss"),
                  ~ replace_na(., 0)) %>%
        select(.id, everything())

division_wins_so_far %>%
        left_join(., division_sim_remaining_wins,
                  by = c("SEASON", "WEEK", "TEAM", "CONFERENCE_GAME", "CONFERENCE_DIVISION_GAME")) %>%
        mutate(sim_total_wins = sim_remaining_wins + wins_so_far, 
               sim_total_loss = sim_remaining_loss + loss_so_far) %>%
        left_join(., conference_divisions,
                  by = c("SEASON", "TEAM")) %>%
        group_by(.id, SEASON, SIM_FROM_WEEK, CONFERENCE, CONFERENCE_DIVISION) %>%
        slice_max(order_by = sim_total_wins,
                  n=1,
                  with_ties = T) %>%
        select(.id, SEASON, TEAM, SIM_FROM_WEEK, CONFERENCE, CONFERENCE_DIVISION, sim_total_wins)
...
midseason_simulations %$% 
        ...
        mutate(CONFERENCE_GAME = case_when(HOME_CONFERENCE == AWAY_CONFERENCE ~ T,
                                           TRUE ~ F),
               CONFERENCE_DIVISION_GAME = case_when(HOME_CONFERENCE == AWAY_CONFERENCE & HOME_CONFERENCE_DIVISION == AWAY_CONFERENCE_DIVISION ~ T,
                                                    HOME_CONFERENCE == AWAY_CONFERENCE & ((is.na(HOME_CONFERENCE_DIVISION) & is.na(AWAY_CONFERENCE_DIVISION))) ~ T,
                                                    TRUE ~ F)) %>%
        filter(CONFERENCE_GAME == T) %>%
   #     filter(CONFERENCE_DIVISION_GAME == T) %>%
      ...
      #  distinct(.id, SEASON, GAME_ID, CONFERENCE, CONFERENCE_DIVISION, TEAM, OPPONENT, winner) %>%
        rename(CONFERENCE = HOME_CONFERENCE,
               ...
               TEAM = HOME_TEAM,
               OPPONENT = AWAY_TEAM) %>%
        select(.id, SIM_FROM_WEEK, GAME_ID, SEASON, CONFERENCE, CONFERENCE_DIVISION, TEAM, OPPONENT, winner) 


            
        group_by(SEASON, SIM_FROM_WEEK, CONFERENCE, CONFERENCE_DIVISION, TEAM) %>%
        count()
        group_by(.id, SEASON, TEAM, SIM_FROM_WEEK, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME) %>%
        slice_max()

division_sim_remaining_wins  = 
        season_weeks_teams %>%
        filter(TEAM == 'Texas A&M') %>%
        left_join(.,
                   sim_remaining_games %>%
                          filter(CONFERENCE_DIVISION_GAME == T) %>%
                          group_by(.id, SEASON, SIM_FROM_WEEK, TEAM, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME, SIM_OUTCOME) %>%
                          count() %>%
                          spread(SIM_OUTCOME, n) %>%
                          mutate(WEEK = SIM_FROM_WEEK),
                  by = c("SEASON", "WEEK", "TEAM")) %>%
        ...
        mutate(sim_total_wins = sim_remaining_wins + wins_so_far, 
               sim_total_loss = sim_remaining_loss + loss_so_far) %>%
        group_by(SEASON, WEEK, TEAM, CONFERENCE_GAME, CONFERENCE_DIVISION_GAME) %>%
        summarize(expected_wins = mean(sim_total_wins))
        group_by(SEASON, WEEK, TEAM, CONFERENCE_GAME, 

```



```{r conference championships}
...
```{r sim conference championships}

sim_conference_championships(m)
...
These results are based on simulating the `r params$input_season` season 1000 times and counting the number of simulations in which a team achieved each outcome.
