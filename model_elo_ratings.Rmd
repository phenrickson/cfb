...
## Example: Simulating the 2001 Season
...
This ends up painting a pretty interesting picture of how a team's season might unfold. In each simulation of a season, a team's Elo rating is updated after each game, which affects how likely they are to win their next game, which affects their Elo rating for the next game, and so on. When we see many lines overlap, this is because there is a typical path based on a team's schedule for how their season is likely to unfold.

Let's zoom into the simulations for Kansas State's 2001 season. I'll plot Kansas State's pregame Elo rating in every simulation across each game in this season. Going into the season, based on their ratings from the last season, they are slight favorites on the road at USC, but it's almost a toss up. The result of this game sends KState on two pretty different paths. Regardless of how they played at USC, they're likely to win against New Mexico State (and only marginally increase their Elo rating). But, in the simulations were they beat USC, they were more likely to beat Oklahoma; when they lost to USC, they were less likely to beat Oklahoma.


```{r look at texas tech 2001}
...
team_temp %>%
        ...
        ggtitle("Simulated Elo Ratings",
                subtitle = str_wrap("Each line is one result from simulating a team's season. Displaying 1000 simulations.", 120))+
        coord_cartesian(ylim = c(1200, 2200))+
        theme(plot.title = element_text(hjust = 0.5),
              plot.subtitle =  element_text(hjust = 0.5))+
        ylab("Pregame Elo Rating")+
        xlab("Game")+
...
sim_team_outcomes %>%
        ...
        scale_color_teams(name = "TEAM")+
        guides(color = 'none')+
        ggtitle("Simulated Wins by Team over Season",
                subtitle = str_wrap("Each line is one result from simulating a team's season. Displaying 1000 simulations.", 120))+
        theme(plot.title = element_text(hjust = 0.5),
              plot.subtitle =  element_text(hjust = 0.5))+
        xlab("Game")+
...
For win totals, we probably mostly care about showing the distribution of wins for each team at the end of the season. 
...
season_totals %>%
        ...
        ggtitle("Simulated Win Totals by Team over Season",
                subtitle = str_wrap(" Displaying total wins by team in 1000 simulations of selected sason.", 120))+
        theme(plot.title = element_text(hjust = 0.5),
              plot.subtitle =  element_text(hjust = 0.5))+
...
season_totals %>%
        ...
        theme(plot.title = element_text(hjust = 0.5),
              plot.subtitle =  element_text(hjust = 0.5))+
        coord_cartesian(ylim = c(0, max(season_totals$n)+50))
