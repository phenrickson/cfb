...
## Playoff and National Championship
...
# bind_rows
playoff_participants = playoff_resumes %>%
        ...
        bind_rows(.,
                  playoff_resumes %>%
                          filter(P5 == 1 & CONFERENCE_CHAMPION == 1 & LOSSES < 2) %>%
        ...
        select(.id, SEASON, TEAM, CONFERENCE, CONFERENCE_CHAMPION, WINS, LOSSES, SCHEDULE, DIFFICULTY, ELO_RANK, POSTGAME_ELO, playoff_priority) %>%
        rename(RATING = POSTGAME_ELO,
               RANK = ELO_RANK) %>%
        arrange(playoff_priority, desc(CONFERENCE_CHAMPION), RANK, desc(RATING), DIFFICULTY, desc(SCHEDULE)) %>%
        mutate(SCHEDULE = round(SCHEDULE, 0),
...
playoff_participants %>%
        ...
        ungroup() %>%
        mutate(perc = n /sims) %>%
        rename(`Pr(Playoff)` = perc) %>%
        select(SEASON, TEAM, CONFERENCE, `Pr(Playoff)`) %>%
        arrange(desc(`Pr(Playoff)`))
