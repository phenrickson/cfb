...
Based on this, we can convert efficiency into Elo ratings and evaluate whether it helps in predicting games and simulating seasons.
...
# 2022 season 
future_season_rankings = expand.grid(TEAM = season_team_rankings  %>%
                                  select(TEAM) %>%
                                  distinct(TEAM) %>%
                                  pull(),
                               TYPE = c('raw', 'adjusted'),
...
teams_data = season_team_rankings %>%
        select(-MARGIN) %>%
        bind_rows(., future_season_rankings) %>%
        filter(TYPE == 'adjusted') %>%
        select(-TYPE) %>%
...
I'm using four features to model each team's Elo rating in season $t$, with the features using information from seasons prior to $t$. I normalized each of these features to indicate the effect of a standard deviation change on next season's Elo rating. 
...
I'll then predict the Elo ratings for next season as the initial values.
...
Compute and compare Elo updated from here.
...
Now, I'll simulate each season based on the adjusted vs actual going into the season.
...
# set.seed(11)
# actual_season_simulations = sim_rest_of_season(games,
