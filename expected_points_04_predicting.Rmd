...
```{r query elo from snowflake}

# query from snowflake
# games
game_elo_ratings = DBI::dbGetQuery(myconn,
                                   "SELECT * FROM CFB_DEMO.CFD_ANALYTICS.ELO_GAME_OUTCOMES")

# teams
team_elo_ratings = DBI::dbGetQuery(myconn,
                                   "SELECT * FROM CFB_DEMO.CFD_ANALYTICS.ELO_TEAM_OUTCOMES")
...
This means we have two different methods for predicting games that perform about the same, one of which (efficiency) is considerably more complicated than the other. Elo ratings also are *much* simpler to use for the purpose of simulating seasons, as we can easily update them based on simulated outcomes. If we were going purely for simplicity and ease of calculation, we'd go for the Elo ratings everytime.
...
weekly_ratings_and_outcomes %>%
        ...
        left_join(.,
                  game_elo_ratings %>%
                          select(GAME_ID, SEASON, HOME_PROB, NEUTRAL_SITE, HOME_PREGAME_ELO, AWAY_PREGAME_ELO) %>%
...
When do they disagree? I'll regress the season end Elo ratings on the season end efficiency measure and look at teams with the largest absolute residuals. This bivariate regression will also tell us the relationship between efficiency and Elo.
...
Season over season, each point of Elo rating from a previous season is worth about .8 for next season's Elo. A team with an average Elo last season will be expected to have an average of about 1531 for next season.
...
Based on this, we can use efficiency to adjust a team's preseason Elo rating - we can then see whether it improves our ability to predict games and simulate seasons.
...
compare = foreach(i = 1:length(seasons),
        ...
        actual_teams_elo = team_elo_ratings %>%
                filter(SEASON < input_season) %>%
                group_by(TEAM) %>%
...
Overall, we're about the same, slightly better on log loss for each season while the accuracy is very close but bounces a bit between seasons.
