...
For the model trained on most of the training set of games, the estimated effect of HOME_ELO_DIFF was about .05. The intercept is a little over 0.65, which indicates the expected point differential in which the two teams have equivalent Elo ratings. This would be higher than if we were not adjusting for the home field advantage in the Elo ratings directly. This would suggest that our adjust isn't nearly enough to capture the entirety of the home field advantage, but we'll stick with it.
...
What was the actual score? Wisconsin 26, Virginia 17, for a point differential of 9. So in this case, the model did kinda nail it, but the point is the uncertainty is generally quite high with our estimates. If we look at all games from week 1, we can see that the model got some right and some wrong - the actual margin is in red, the prediction interval is in black.
...
Here's what the picture for Illinois looks like after their first few games. At the start of the season they were expected to lose to Louisville by a touchdown, but Illinois pulled off an upset at California in week one and then won their game against Northern Illinois. Now, going into Louisville, they are now slight favorites, and they are slightly more likely to win the rest of their matchups.
...
So far, everything has been built to predict upcoming games and seasons using nothing more than a tuned Elo rating that accounts for home field advantage and reversion year over year. The results are kind of cool given the simplicity of this approach, but there's at least one elephant in the room that I want to add to the model: team recruiting ratings. 
...
A team's recruiting ranking is scored year by year, but a team's roster is composed of players from the last few years. This means instead of just using recruiting scores, we should probably use some sort of weighted moving average.
...
Here I'll show each SEC team's recruiting points by year, as well as two different forms of weighted moving averages, exponentially decaying weights and manually selected weights. 
...
This distribution seems to be shifting over time too.

```{r looking at distributions over time for the smoothed and nnot}
...
Next, I'll get each team's recruiting score in the lead up to these seasons. I only have a rolling recruiting score for each team starting in 2007, so I'll be looking at the years 2007-2010 in particular here. Recruiting overlaps with the season, so for a team's recruiting score I'll use a moving average of their previous (four) years, not including the current year. This means each team's recruiting score will be constant throughout the season. (Yes, I am aware of injuries and transfers, but this is a start.)

Each team will have an Elo rating at the **start** of the season, which is based on their previous seasons, and an Elo rating at the **end** of the season. For the years 2007-2010, I'll regress their **season end Elo** on their **season start Elo** and their **recruiting score**. If a team's recruiting score has a (positive) effect on a team's season end Elo, conditional on their starting Elo, we'll have some evidence in favor of adding a recruiting adjustment at the beginning of each season.
...
This is the correlation between a team's starting and ending elo in each of these seasons.
...
Now regress Elo End on Elo Start, with and without recruiting. To make this a bit easier to interpret, I'll center the predictors at zero so that the intercept will indicate the average Elo End. 

The first model looks at predicting a team's season ending Elo using just its starting Elo. The model indicates that a team with an average Elo at the start of the year will end up with a rating of 1577, with each additional point above on starting Elo the mean increasing their ending Elo by .93. This should make sense: where you end up is on average a function of where you start, and knowing your starting Elo explains about 77% of the ending Elo.
...
without_recruiting_lm %>%
        ...
        unnest() %>%
        mutate_if(is.numeric, round, 3) %>%
        select(r.squared, sigma, no)

```
Now, looking at the model that includes recruiting, we can see that the coefficient for Elo Start has dropped slightly, down to .8, and the coefficient on Elo is .6. This means, functionally, that each additional point above the average on a team's recruiting score is worth an additional .6 Elo points at the end of the season.
...
## Effects of Recruiting on Point Differential and Wins

How can we use this in an Elo model? My initial thihnkings is that we evaluate giving each team a bump at the start of the season based on the talent on their roster (as proxied for by their rolling composite score). A team full of young, talented recruits might not have won many games, but the talent on their roster would make us think that they would be due to break through soon. We would expect teams that have been building talented rosters to punch above their Elo weight in future seasons.

I only mention giving a boost to Elo at the start of the season because eventually a team will start to demonstrate its value on the field and their Elo rating will catch up. The question is whether we can adjust ratings at the beginning of the season so as to catch onto these types of teams faster.


Here's how I'll test this out. 

1. Compute Elo ratings through 2010
2. Train point model through 2006
3. Compute each team's recruiting score (an exponentially weighted moving average of their previous year's classes)
4. Predict winner and point differential for years 2007-2012
5. Evaluate whether the residuals are correlated with the difference in the two team's recruiting rating.
...
```{r look at correlation between start of season recruiting and end of season elo}

foo = elo_train$team_outcomes %>%
    filter(SEASON > 2006) %>%
    left_join(., recruiting_teams_averages,
              by = c("TEAM", "SEASON", "CONFERENCE")) %>%
        mutate_at(c("WMA", "EMA"),
        ...
        mutate(ELO = case_when(FIRST_GAME ==T ~ PREGAME_ELO,
                               LAST_GAME==T ~ POSTGAME_ELO)) %>%
        mutate(GAME = case_when(FIRST_GAME ==T ~ 'start',
                                LAST_GAME == T ~ 'end')) %>%
        ungroup() %>%
        select(SEASON, TEAM, CONFERENCE, GAME, ELO, WMA, EMA) %>%
        spread(GAME, ELO)
...
bar = foo %>%
        mutate(start = start - mean(start)) %>%
        mutate(EMA = EMA -mean(EMA),
               WMA = WMA - mean(WMA)) %>%
        nest() %>%
        mutate(lm = map(data, ~ lm(end ~ start + EMA,
                                   data = .x))) %>%
...
regress_recruiting_on_elo = elo_train$team_outcomes %>%
        filter(SEASON > 2006) %>%
        left_join(., recruiting_teams_averages,
                  by = c("TEAM", "SEASON", "CONFERENCE")) %>%
        group_by(SEASON, TEAM) %>%
        mutate(obs = row_number()) %>%
        mutate(game = case_when(obs == min(obs) ~ 'first game of season',
                                obs == max(obs) ~ 'last game of season')) %>%
        mutate()
        ungroup() %>%
        filter(game == 'last game of season') %>%
        filter(!is.na(game)) %>%
        nest(-SEASON) %>%
        mutate(lm = map(data, ~ lm(POSTGAME_ELO ~ EMA,
                                   data = .x))) %>%
        mutate(tidied = map(lm, tidy, se='robust', conf.int=T)) %>%
        mutate(glanced = map(lm, glance)) 

regress_recruiting_on_elo %>%
        select(SEASON, tidied) %>%
        unnest() %>%
        mutate_if(is.numeric, round, 3)
...
elo_train$team_outcomes %>%
        ...
        filter(game == 'last game of season') %>%
        filter(!is.na(game)) %>%
        ggplot(., aes(x=POSTGAME_ELO,
                      label = paste(TEAM, SEASON),
                      y=EMA))+
        geom_point()+
        geom_text(check_overlap = T,
                  vjust = -1)+
        facet_wrap(game+SEASON ~.,
                   ncol = 2)+
...
Now, I'll fit a regression on the point differential on the training set.

```{r fit points model through 2006}
...
points_model = elo_train$game_outcomes %>%
        filter(SEASON >=1975) %>%
        mutate(HOME_PREGAME_ELO = case_when(NEUTRAL_SITE == T ~ HOME_PREGAME_ELO -selected_pars$home_field_advantage,
...
Now, I'll compute Elo ratings through 2012.

```{r update elo through 2012, include =F}

valid_games = games_data_tidied %>%
        filter(SEASON >=2007 & SEASON <=2012) %>%
        arrange(GAME_DATE) %>%
        select(GAME_ID, 
               SEASON, 
               WEEK,
...
Then, I'll predict the point differential for these games based on their Elo ratings.

```{r predict 07 to 2012}

valid_prepped = elo_valid$game_outcomes %>%
                              mutate(HOME_PREGAME_ELO = case_when(NEUTRAL_SITE == T ~ 
                                                                          HOME_PREGAME_ELO -selected_pars$home_field_advantage,
                                                                  TRUE ~ HOME_PREGAME_ELO)) %>%
                              mutate(HOME_ELO_DIFF = HOME_PREGAME_ELO - AWAY_PREGAME_ELO) %>%
                              mutate(HOME_SCORE_DIFF = HOME_POINTS-AWAY_POINTS) %>%
        left_join(.,
                  recruiting_teams_averages %>%
                          ...
                          rename(HOME_TEAM = TEAM,
                                 HOME_RECRUITING_POINTS = RECRUITING_POINTS,
        ...
        left_join(.,
                  recruiting_teams_averages %>%
                          ...
                          rename(AWAY_TEAM = TEAM,
                                 AWAY_RECRUITING_POINTS = RECRUITING_POINTS,
                                 AWAY_RECRUITING_EMA = EMA,
                                 AWAY_RECRUITING_WMA = WMA),
                  by = c("SEASON", "AWAY_TEAM")) %>%
        mutate_at(c("HOME_RECRUITING_POINTS",
                    "HOME_RECRUITING_EMA",
                    "HOME_RECRUITING_WMA",
...
```{r look at effect of recruiting ratings on game outcomes}

valid_preds = predict(points_model,
                      newdata = valid_prepped,
                      interval = 'prediction') %>%
        bind_cols(., valid_prepped)

valid_preds %>%
        mutate(resid = HOME_SCORE_DIFF-fit) %>%
        mutate(row = row_number()) %>%
        ggplot(., aes(y=resid,
                      x=HOME_RECRUITING_DIFF))+
        geom_point()+
        stat_cor(p.accuracy = 0.01)+
        geom_smooth(method = 'lm',
                    formula = 'y ~ x')+
...
Okay so the difference in recruiting scores is correlated with the residuals from our point differential model, which uses pre game Elo scores.

```{r compare these and the point difference}

valid_preds %>%
        left_join(., games_data_tidied %>%
                          select(GAME_ID, CONFERENCE_GAME),
                  by = c("GAME_ID")) %>%
        ggplot(., aes(x=HOME_RECRUITING_DIFF,
                      label = paste(HOME_TEAM, "-", AWAY_TEAM, ",", SEASON, sep=""),
                    #  color = CONFERENCE_GAME,
                      y = HOME_ELO_DIFF))+
        geom_text(check_overlap = T,
                  vjust = -1,
                  size =3)+
...
```{r fit model on those years}

compare_models = 
        bind_rows(valid_prepped %>%
                          nest() %>%
                          mutate(lm = map(data, ~ lm(HOME_SCORE_DIFF ~ HOME_ELO_DIFF,
                                                     data = .x))) %>%
                          mutate(method = 'Elo'),
                  valid_prepped %>%
                   ...
                   valid_prepped %>%
                          mutate(HOME_WIN = factor(case_when(HOME_POINTS > AWAY_POINTS ~ 'yes',
                                                             TRUE ~ 'no'))) %>%
                          nest() %>%
                          mutate(lm = map(data, ~ lm(HOME_SCORE_DIFF ~ HOME_RECRUITING_DIFF,
                                                     data = .x))) %>%
                          mutate(method = 'Recruiting'))
...
# %>%
#         filter(term == 'HOME_RECRUITING_DIFF') %>%
#         ggplot(., aes(x=WEEK,
#                       color = method,
#                       y=estimate,
#                       ymin = conf.low,
...
valid_prepped %>%
        ...
        unnest() %>%
        mutate_if(is.numeric, round, 3) %>%
        select(WEEK, term, estimate, conf.low, conf.high)  
