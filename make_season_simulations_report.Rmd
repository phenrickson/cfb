...
How has each team's trajectory changed throughout the season? After each week of games have concluded, I update each team's rating and simulate all remaining games in the season. 

The following table shows how each team's rating and predictions have changed after each week of the season, highlighting the largest absolute changes from week `r params$sim_week`.

```{r changes}

probs_change_df = data.frame(breaks = seq(-.9, .9, .05))

wins_change_df = data.frame(breaks = seq(-3, 3, .1))

elo_change_df = data.frame(outcome = 'Rating',
                           breaks = seq(-150, 150, 5))

blueOrangeColorRamp = colorRampPalette(c("orange", "white", "dodgerblue3"))
                          
team_predictions %>%
        arrange(SEASON, SIM_FROM_WEEK, TEAM) %>%
        mutate(TEAM = case_when(params$input_season == 2022 ~ paste('<a href="https://phenrickson.github.io/cfb/team_profiles/preseason/', params$input_season, "/", TEAM, '.html','">', TEAM, '</a>', sep=""),
                                TRUE ~ TEAM)) %>%
        group_by(SEASON, TEAM) %>%
        mutate(rating_diff = RATING - dplyr::lag(RATING, 1),
               win_diff = mean_wins - dplyr::lag(mean_wins, 1),
               made_bowl_diff = made_bowl - dplyr::lag(made_bowl, 1),
               made_conf_diff = made_conf - dplyr::lag(made_conf, 1),
               made_nc_diff = made_nc - dplyr::lag(made_nc, 1),
               win_conf_diff = win_conf - dplyr::lag(win_conf, 1),
               made_playoff_diff = made_playoff - dplyr::lag(made_playoff, 1),
               win_nc_diff = win_nc - dplyr::lag(win_nc, 1)) %>%
        select(SEASON, CONFERENCE, SIM_FROM_WEEK, TEAM, contains("_diff")) %>%
        ungroup() %>%
        filter(SIM_FROM_WEEK != 0) %>%
   #     filter(SIM_FROM_WEEK == params$sim_week) %>%
        arrange(made_playoff_diff) %>%
        select(SEASON, SIM_FROM_WEEK, TEAM, everything()) %>%
        arrange(desc(made_playoff_diff)) %>%
        arrange(desc(SIM_FROM_WEEK), desc(abs(rating_diff))) %>%
        mutate(SIM_FROM_WEEK = factor(SIM_FROM_WEEK)) %>%
        mutate(CONFERENCE = factor(CONFERENCE)) %>%
        transmute(Week = SIM_FROM_WEEK,
                  Conf = CONFERENCE,
                  Team = TEAM,
                  Rating = round(rating_diff, 0),
                  Wins = round(win_diff, 1),
                  Bowl = made_bowl_diff,
             #     Made_Conf = made_conf_diff,
                  Win_Conf = win_conf_diff,
                  Playoff = made_playoff_diff,
                  NC = made_nc_diff) %>%
              #    Win_NC = win_nc_diff) %>%
        mutate_at(c("Bowl",
                 #   "Make_CC",
          ...
          datatable(escape=F,
                ...
                formatStyle(c(
                        ...
                        "NC"),
                        #"Win_NC"),
                    backgroundColor = styleInterval(probs_change_df %>%
                                                            pull(breaks),
                                                    blueOrangeColorRamp(probs_change_df %>%
                                                                          pull(breaks) %>%
                                                                          length()+1))) %>%
        formatStyle(c("Rating"), 
                    backgroundColor = styleInterval(elo_change_df %>%
                                                            pull(breaks),
                                                    blueOrangeColorRamp(elo_change_df %>%
                                                                          pull(breaks) %>%
                                                                          length()+1))) %>%
                formatStyle(c("Wins"), 
                    backgroundColor = styleInterval(wins_change_df %>%
                                                            pull(breaks),
                                                    blueOrangeColorRamp(wins_change_df %>%
                                                                          pull(breaks) %>%
                                                                          length()+1)))

```

<!-- Which teams improved or decreased the most in Week `r params$sim_week`? -->

<!-- ```{r plot team rating changes, fig.height=6} -->
...
<!--         filter(SIM_FROM_WEEK != 0) %>% -->
<!--         filter(SIM_FROM_WEEK == params$sim_week) %>% -->
<!--         slice_max(abs(rating_diff), n=40, with_ties = F) %>% -->
<!--         ggplot(., aes(x=rating_diff, -->
<!--                       fill = rating_diff, -->
<!--                       y=reorder(TEAM, rating_diff)))+ -->
<!--                 geom_col()+ -->
<!--         theme_phil()+ -->
<!--         coord_cartesian(xlim = c(-150, 150))+ -->
<!--         guides(color = "none")+ -->
<!--         xlab("\u394 Rating from Previous Week")+ -->
<!--         ylab("")+ -->
<!--         scale_fill_gradient2(low = 'orange', -->
<!--                              mid = 'white', -->
<!--                              midpoint = 0)+ -->
<!--         guides(fill = "none")+ -->
<!--         ggtitle(paste("Biggest Team Rating Changes in Week", params$sim_week), -->
<!--                         subtitle = paste("Displaying teams with the largest absolute changes in rating after the results of Week", params$sim_week, "\n"))+ -->
...
<!--         select(analysis, id) %>% -->
<!--         unnest(analysis) %>% -->
<!--         mutate(prob = plyr::round_any(HOME_PROB, .1, round)) %>% -->
<!--         group_by(SEASON, id, prob, HOME_OUTCOME) %>%  -->
<!--         count() %>% -->
<!--         spread(HOME_OUTCOME, n) %>% -->
<!--         mutate_at(c("win", "loss"), replace_na, 0) %>% -->
<!--         mutate(n = win + loss) %>% -->
<!--         mutate(observed = win / n) %>% -->
<!--         group_by(SEASON, prob)  %>% -->
<!--         summarize(mean = mean(observed), -->
<!--                   n = mean(n), -->
<!--                   quantile = quantile(observed, c(0.1, 0.5, .9)), q = c("lwr", "mid", "upr"), -->
<!--                   .groups = 'drop') %>% -->
<!--         spread(q, quantile) %>% -->
<!--         ggplot(aes(x=prob, -->
<!--                    y = mid, -->
...
<!--                         xlim = c(0, 1))+ -->
<!--         xlab("Predicted Pr(Home Win)")+ -->
<!--         ylab("Observed Pr(Home Win)")+ -->
<!--         theme_phil()+ -->
<!--         theme(legend.title = element_text(size = 8))+ -->
<!--         guides(size = guide_legend(title.position = 'top', -->
<!--                                    title = 'Number of Games', -->
<!--                                    title.hjust = 0.5))+ -->
<!--         ggtitle(paste(params$input_season, "Model Calibration as of Week", params$sim_week), -->
<!--                 subtitle = str_wrap(paste("Displaying observed probability of home win vs predicted probability of home wins from model. 80% percentile intervals around each estimate from 1000 bootstraps."), 90))+ -->
<!--         theme(plot.title = element_text(hjust = 0.5, size = 16), -->
<!--               plot.subtitle =  element_text(hjust = 0.5), -->
<!--               strip.text.x = element_text(size = 12)) -->
...
How are teams performing relative to preseason expectations? The following visualizations show every team's expected regular season wins as of Week `r params$sim_week` compared to their expected regular seasons from the beginning of the season. Teams that are performing better than expected will be **above** the grey dashed line; teams that are performing worse than expected will be **below** the grey dashed line.

Note: these visualizations will only display once a season is underway.

## Current vs Preseason

```{r display expected wins vs preseason expected wins, fig.height=8}

set.seed(1)
pos = position_jitternormal(sd_x = .03,
                            sd_y = .025)
...
team_predictions %>%
     ...
     #   mutate(label_TEAM = case_when(abs(current - begin) >= 1 ~ TEAM)) %>%
        ...
        custom_color_teams+
        guides(color = 'none')+
        scale_y_continuous(limits = c(-1, 12.5),
                           breaks = seq(0, 12, 3))+
        ...
        ggtitle(paste(params$input_season, "Current vs Preseason Predicted Win Totals", params$sim_week),
                subtitle = str_wrap(paste("Displaying each team's average predicted win total in simulations of remaining games, current vs preseason. \n Points jittered slightly to improve visibility."), 120))+
        theme(plot.title = element_text(hjust = 0.5, size = 16),
              plot.subtitle =  element_text(hjust = 0.5),
              strip.text.x = element_text(size = 12))
